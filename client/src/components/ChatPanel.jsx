  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      <div className="mb-2 md:mb-4 shrink-0 px-1">
        <h2 className="text-base md:text-lg font-bold tracking-tight text-slate-900 dark:text-white drop-shadow-sm transition-colors">Room Chat</h2>
        <p className="mt-0.5 md:mt-1 text-[10px] md:text-xs font-medium text-slate-500 transition-colors">Talk with your friends</p>
      </div>

      <div className="flex flex-1 flex-col gap-2 md:gap-3 overflow-y-auto px-1 custom-scrollbar transition-colors min-h-0">
        {messages.length === 0 ? (
          ...
          messages.map((message, index) => (
            <div key={index} className="rounded-xl md:rounded-2xl rounded-tl-sm border border-slate-200 dark:border-white/5 bg-slate-50/80 dark:bg-white/[0.03] p-2 md:p-3 shadow-sm transition-colors w-[90%] self-start">
              ...
              <p className="mt-0.5 md:mt-1 text-xs md:text-sm text-slate-700 dark:text-slate-200 leading-relaxed break-words">
                {message.message}
              </p>
            </div>
          ))
      ...
      <form className="mt-2 md:mt-4 shrink-0 px-1 pb-1 md:pb-2" onSubmit={handleSubmit}>
        <div className="relative flex items-center">
          <button
            type="button"
            onClick={toggleMute}
            disabled={micStatus !== 'Connected'}
            className={`absolute left-1 md:left-1.5 flex h-8 w-8 md:h-9 md:w-9 items-center justify-center rounded-lg md:rounded-xl transition-all active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed ${
              muted 
          ...
          <input
            ...
            maxLength={300}
            className="h-10 md:h-12 w-full rounded-xl md:rounded-2xl border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-black/40 pl-[2.75rem] md:pl-[3.25rem] pr-12 md:pr-14 text-xs md:text-sm text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 outline-none transition-all focus:border-indigo-500 dark:focus:border-indigo-500/50 focus:bg-white dark:focus:bg-black/60 focus:ring-2 focus:ring-indigo-500/20"
          />
          <button
            type="submit"
            disabled={!chatInput.trim()}
            className="absolute right-1 md:right-1.5 flex h-8 w-8 md:h-9 md:w-9 items-center justify-center rounded-lg md:rounded-xl bg-indigo-600 font-bold text-white transition-all hover:bg-indigo-500 active:scale-95 disabled:opacity-50 disabled:scale-100 disabled:cursor-not-allowed shadow-sm"
          >
